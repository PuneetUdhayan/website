...
function App() {
  ...
  useEffect(() => {
    ...
    const animationDuration =
      (nameText.length + intro.length + options?.[0].length) * 0.03 + 0.04 * 1 + 0.5; // Roughly based on your stagger and delay
    const timer = setTimeout(() => {
    ...
    <div>
      <div className="grid grid-cols-1 md:grid-cols-2">
        <div className="h-screen flex items-center">
          <div>
             <div><LetterAnimation text={"I am Puneet Udhayan."} /></div>
          {nameAnimationComplete && !animationComplete && (
            <div><LetterAnimation text={intro + options?.[0]} /></div>
          )}
          {nameAnimationComplete && animationComplete && (
